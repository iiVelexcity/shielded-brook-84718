...
bot.on('message', message => {
    ...
    // Ping
    if (msg === prefix + 'VELEX') { // This checks if msg (the message but in all caps), is the same as the prefix + the command in all caps.
        message.channel.send('<@!410203298621161483> is the best! We all know he is the best owner'); // This 'sends' the message to the channel the message was in. You can change what is in the message to whatever you want.
    }
    // Purge
    if (msg.startsWith(prefix + 'PURGE')) { // This time we have to use startsWith, since we will be adding a number to the end of the command.
        ...
        async function purge() {
            ...
            if (!message.member.roles.find("name", "[HEAD] Administrator")) { // This checks to see if they DONT have it, the "!" inverts the true/false
                message.channel.send('You need the \`[HEAD] Administrator\` role to use this command.'); // This tells the user in chat that they need the role.
            ...
            message.channel.send(fetched.size + ' Messages were deleted.')
    ...
    if (msg.startsWith(prefix + 'WEATHER')) { // This checks to see if the beginning of the message is calling the weather command.
        ...
        if (!message.mentions.members.first()) {
            return message.reply("No user mentioned.").then(m => m.delete({timeout: 5000}));
        } else {
            member.kick().then((member) => {
            message.channel.send(`${member} has been kicked.`);
            }).catch(() => {
            message.channel.send("You do not have the permissions to kick this member. If you should and it is not working do not hesitate to dm iiVelexcity");
    ...
    if (msg.startsWith(prefix + 'BAN')) {
      ...
      if (!message.mentions.members.first()) {
        ...
        member.ban().then((member) => {
          message.channel.send(`BANG BANG ${member} was a bad boy and he just got punished. Won't see you around here anymore cowyboy. That is a PBAN! HOO RA`);
        }).catch(() => {
          message.channel.send("You do not have the permissions to BAN this member. If you should and it is not working do not hesitate to dm iiVelexcity");
    ...
    bot.on(`guildMemberAdd` , member => {
      ...
      .setColor('RANDOM')
      .setDescription(member.user.tag + "Just joined the Official TRGD Discord Server! Make sure you read all of the rules and if you need help, Dont hesitate to ask any HR (13+)")
    ...
    bot.on(`guildMemberRemove` , member => {
      ...
      var embed = new Discord.RichEmbed()
      .setColor('RANDOM')
      .setDescription(member.user.tag + "Just left the Official TRGD Discord Server. Wish them GL on their journey without us!")
        leave.send(embed)
});

bot.on('ready', () => {
    console.log('The amazing Bot has started! Get ready for some fun. Thanks to iiVelexcity, This BOT IS AWESOME');
});
bot.login('NDE1NjI2MjI3NTU5MTcwMDcw.DXMs6g.M50MNjujVoMlDa8NEUiN732WC-k')
